...
class Megadepth1500BenchmarkTestOpt:
        ...
        with torch.no_grad():
            ...
            for scene_ind in range(len(self.scenes)):
                ...
                for pairind in tqdm(pair_inds):
                    ...
                    # print('before', w1, h1, w2, h2)
                    w1, h1 = scale1 * w1, scale1 * h1
                    w2, h2 = scale2 * w2, scale2 * h2
                    # print('after', w1, h1, w2, h2)
                    ...
                    kpts1_vis = kpts1.cpu().numpy() / scale1
                    kpts2_vis = kpts2.cpu().numpy() / scale2
                    kpts1_vis = [cv2.KeyPoint(x[0], x[1], 1) for x in kpts1_vis]
                    kpts2_vis = [cv2.KeyPoint(x[0], x[1], 1) for x in kpts2_vis]

                    im1_name = os.path.basename(im_paths[idx1])[:-4]
                    im2_name = os.path.basename(im_paths[idx2])[:-4]
                    matched_img_path = os.path.join(res_dir, f'{im1_name}_{im2_name}.png')
                    matched_img = cv2.drawMatches(img1, kpts1_vis, img2, kpts2_vis, [], None)
                    cv2.imwrite(matched_img_path, matched_img)

                    matched_img_path = os.path.join(res_dir, f'{im1_name}_{im2_name}_matches.png')
